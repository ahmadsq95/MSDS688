...
def path_length(graph, path):
    ...
    l = False
    while x < len(path) - 1:
        if graph.is_neighbor(path[x],path[x+1]):
            l+= graph.get_edges(path[x],path[x+1]).length
        x+=1
    return l
...
def basic_bfs(graph, startNode, goalNode):
    ...
    while len(pathlist) > 0:    
            ...
            while len(pathlist) > 0:
                ...
                pathlist.remove(pathlist[0])
                nodeToExtend = pathToExtend[-1]
                newNode = graph.get_connected_nodes(nodeToExtend)
                if len(pathToExtend) > 1:
                    newNodes = [node for node in newNode if node not in pathToExtend]
                if goalNode in newNodes:
                    goalPath = pathToExtend + (goalNode, )
                    return list(goalPath)
                newPath += [pathToExtend + (node,) for node in newNodes]
...
def is_admissible(graph, goalNode):
    ...
    for node in graph.nodes:
        ...
        pathToGoal = a_star(graph,node,goalNode)
        if len(pathToGoal) > 0:
            distanceToGoal = a_star(graph,pathToGoal)
            if graph.get_heuristic_value(node, goalNode) > distanceToGoal:
                admis = False
...
def is_consistent(graph, goalNode):
    ...
    for edge in graph.get_edges:
        if abs(graph.get_heuristic_value(edge.node2,goalNode) - graph.get_heuristic_value(edge.node1, goalNode)) > edge.length:
            consi = False
    return consi
...
WHAT_I_FOUND_INTERESTING = "That DFS and BFS are similar"
